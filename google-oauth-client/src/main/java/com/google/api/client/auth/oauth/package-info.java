 ...
 *
 * <ul>
 *   <li>{@code PLAINTEXT} signature algorithm
 *   <li>{@code "application/x-www-form-urlencoded"} HTTP request body
 ...
 * </ul>
 *
 * <p>Before using this library, you may need to set up your application as follows:
 *
 * <ol>
 *   <li>For web applications, you may need to first register your application with the
 *       authorization server. It may provide two pieces of information you need:
 *       <ul>
 *         <li>OAuth Consumer Key: use this as the {@code consumerKey} on every OAuth request, for
 *             example in {@link
 ...
 * </ol>
 *
 * <p>After the set up has been completed, the typical application flow is:
 *
 * <ol>
 *   <li>Request a temporary credentials token from the Authorization server using {@link
 ...
 *       <ul>
 *         <li>For web applications, the user's browser will be redirected to the callback URL which
 *             may be parsed using {@link com.google.api.client.auth.oauth.OAuthCallbackUrl}.
 *         <li>For installed applications, see the authorization server's documentation for figuring
 *             out the verification code.
 *       </ul>
 *   <li>Request to exchange the temporary token for a long-lived access token from the
 *       Authorization server using {@link com.google.api.client.auth.oauth.OAuthGetAccessToken}.
 *       This access token must be stored.
 *   <li>Use the stored access token to authorize HTTP requests to protected resources by setting
